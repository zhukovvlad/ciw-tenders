...
Откройте в консоли Neon вкладку **SQL Editor** (для ветки `main`) и выполните:
...
## 4. Применить миграцию (создать таблицу)

Способ А — через `psql` (если установлен):

```bash
psql "postgresql://<user>:<password>@<endpoint>.../neondb?sslmode=require" -f backend/migrations/001_init.sql
```

> В `psql` используйте обычную схему `postgresql://` (без `+psycopg` — это только для SQLAlchemy).

Способ Б — без psql: откройте `backend/migrations/001_init.sql`, скопируйте содержимое
в **SQL Editor** Neon и выполните.
...
4. Выполните на ветке `test`:
   ...
   CREATE EXTENSION IF NOT EXISTS vector;
   ```
   и примените миграцию (см. §4), указав строку ветки `test`.
...
- [ ] Миграция `001_init.sql` применена, таблица `template_articles` создана.
