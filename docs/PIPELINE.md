...
### Движок `tree` (за флагом `MATCHING_ENGINE=tree`)

Альтернативная архитектура матчинга: вместо построчного RAG чанкирует смету по разделам и отправляет
каждый чанк LLM со **всем справочником в промпте** (кэшируется через prompt caching). Ядро то же
(выбор статьи из каталога), но модель видит контекст: родителя, соседей, полную структуру.
Одна смета → 18–40 вызовов вместо ~200 построчных вызовов LLM-арбитра у RAG (в RAG на 809-строчной
смете большинство round-trip'ов — это не LLM, а pgvector `search_similar`/`save_node_match` на каждый
узел, см. [TECH_DEBT.md](TECH_DEBT.md)). Вердикты с самооценкой модели `sure`, затем структурная
валидация: **fail-closed по двум карманам, не одному** — отсутствующий вердикт (модель не ответила на
цель) → `error`; malformed/inconsistent/unknown_code вердикт → `needs_review` с `matched_* = NULL` и
кандидатами: сама доверенная статья предка и её прямые дети (не более 5). Ни один из путей не даёт молчаливый
`confident` — разница в том, что сломанный, но существующий ответ обязан прийти к оператору с
кандидатами на выбор, а не осесть в техническом `error` без кандидатов
(`backend/app/domain/tree_matching.py:296-308`).
Использует те же имена статусов, что и таблица выше, но по другим критериям — например, `confident`
у tree-движка означает `sure=true` без флагов валидации и без барьера needs_review в цепочке предков,
а не порог по `score` (score у tree-движка всегда `null`, косинуса нет). Фонд решений (§6 спеки)
отложен на отдельный PR. Дефолт пока RAG (`matching_engine='rag'`); движок включается явным
переключением конфига. Спека и результаты спайка (94.9% top-1 vs 77% RAG на бенчмарке):
[2026-08-27-tree-matching-core-design.md](superpowers/specs/2026-08-27-tree-matching-core-design.md).
