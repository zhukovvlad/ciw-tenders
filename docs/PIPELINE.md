...
Одна смета → 18–40 вызовов вместо ~1200 построчных. Вердикты с самооценкой модели `sure`, затем
структурная валидация (fail-closed: любая ошибка данных → `error`, никогда молча `confident`).
