...
- Produces:
  ...
  - `saveReview(state: ReviewState): void` (тихо игнорирует `QuotaExceededError`)
  - `loadReview(): ReviewState | null`
  - `clearReview(): void`

> Спека: для наблюдаемого диапазона (десятки–сотни строк) `sessionStorage` достаточно; перенос на IndexedDB — только если реальные сметы дойдут до тысяч строк (решает шаг 0). Здесь — `sessionStorage`, с безопасной обработкой переполнения.
...
export function saveReview(state: ReviewState): void {
  try {
    ...
    // QuotaExceededError на крупных сметах — тихо игнорируем (см. спеку: перенос на IndexedDB при тысячах строк)
  }
}
...
export function ReviewScreen({ state, dispatch, onExport, onNewEstimate }: ReviewScreenProps) {
  ...
  const flaggedRows = state.rows.filter((r) => r.status !== "confident")
  const gotoNext = () => {
    const idx = flaggedRows.findIndex((r) => r.row_number === activeRow)
    const next = flaggedRows.slice(idx + 1).find((r) => decisionFor(state, r).kind === "pending")
      ?? flaggedRows.find((r) => decisionFor(state, r).kind === "pending")
    setActiveRow(next ? next.row_number : null)
  }
  // автостарт: первая нерешённая спорная
  useEffect(() => {
    if (activeRow === null) {
      const first = flaggedRows.find((r) => decisionFor(state, r).kind === "pending")
      if (first) setActiveRow(first.row_number)
    }
  }, [activeRow, flaggedRows, state])
...
export function EstimateFlow() {
  ...
  async function handleFile(file: File) {
    ...
    // переинициализация ревью новыми строками
    const fresh = initReview(file.name, rows)
    saveReview(fresh)
    // перезагрузка reducer через restored-канал: проще пере-маунт через key (см. App), здесь — dispatch замены
    Object.assign(state, fresh) // безопасно: следом setPhase вызовет ререндер; для чистоты см. примечание
    setPhase("review")
  }
...
> **Примечание по реинициализации reducer:** мутация `Object.assign(state, fresh)` — упрощение для плана. Чистая реализация: добавить в `ReviewAction` вариант `{ type: "load"; state: ReviewState }`, который возвращает `action.state`, и вызывать `dispatch({ type: "load", state: fresh })` вместо `Object.assign`. Реализуйте чистый вариант: добавьте в `reviewState.ts` (Task 5) ветку `case "load": return action.state` и тип в `ReviewAction`, затем здесь — `dispatch({ type: "load", state: fresh })`. Тест Task 5 не ломается (новая ветка аддитивна).

- [ ] **Step 4: Применить чистую реинициализацию**

В `reviewState.ts` добавить в `ReviewAction` `| { type: "load"; state: ReviewState }` и в `reviewReducer` ветку `case "load": return action.state`. В `EstimateFlow.handleFile` заменить `Object.assign(state, fresh)` на `dispatch({ type: "load", state: fresh })`.
...
```bash
git add src/pages/estimate/EstimateFlow.tsx src/lib/reviewState.ts
...
- Долговечность сессии (sessionStorage, guard, очистка при «новой смете») → Tasks 4, 11.
