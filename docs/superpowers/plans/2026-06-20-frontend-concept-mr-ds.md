...
- Виртуализация — **не реализуется в прототипе**, потому что он гоняет 15 фикстур (объём заведомо крошечный), а НЕ по разрешению спеки и НЕ по «наблюдаемому диапазону» (замера распределения/размеров не было — см. гейт спеки, шаг 0). **Production-гейт:** прод обязан добавить виртуализацию per спека (раздел «Производительность таблицы»), включая приёмочный критерий «фокус уезжает на следующую спорную, даже если она вне отрендеренного окна». Не читать как «спека разрешает обойтись без виртуализации».
...
**Скан плейсхолдеров:** код приведён полностью в каждом шаге; реинициализация reducer реализована чисто через action `load` (Task 5), без временных мутаций — отдельного «доделочного» шага не осталось.
