...
- **Per-node идентичность (сквозной инвариант):** дубли кодов раздела между этапами НЕ схлопываются нигде — `source_index` уникален на строку и протянут `seed → EstimateNode → estimate_rows → DTO`. Сопоставление gold↔результат в `_report` идёт по `source_index`, НЕ по `code` (иначе дубли слипаются и метрика врёт).
- **Логирование (конвенция проекта):** оба скрипта зовут `setup_logging()` на входе (как `create_admin`/`smoke_import`). **Вывод программы — `print()`/`input()`** (сводка метрик, интерактивное подтверждение `no_article`) — это результат/I/O, не диагностика; статус-сообщения — `logger`. С `setup_logging()` харнесс **бесплатно** получает наблюдаемость уже инструментированного пайплайна: per-estimate summary матчинга + латентность/попытки AI-вызовов (арбитр/эмбеддер) во время прогона.
