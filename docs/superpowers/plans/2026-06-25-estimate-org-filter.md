...
| `backend/app/domain/ports.py` | Порт `WorkTypeClassifier`; методы `fetch_all_nodes`/`apply_node_classification` | Modify |
...
- Produces:
  ...
  - `EstimateRepository.apply_node_classification(node_id, *, excluded: bool, embedding_input: str) -> None` — всегда обновляет `embedding_input`; при `excluded=True` ставит `status='excluded'`, иначе status не трогает.
  - `fetch_unembedded_nodes` дополнительно фильтрует `status != 'excluded'`; `nodes_count` в `list_for_owner` считает `status != 'excluded'`.
  - (`fetch_matchable_nodes`/`count_unfinished_nodes`/`count_node_errors` менять НЕ нужно — `excluded` не входит в их наборы статусов.)
...
def test_fake_repo_excludes_marked_org() -> None:
    ...
    repo.apply_node_classification(nid, excluded=True, embedding_input="1 Этап ЖК")
...
def test_fake_repo_survivor_keeps_pending_and_new_breadcrumb() -> None:
    ...
    repo.apply_node_classification(nid, excluded=False, embedding_input="Чистая крошка")
...
Expected: FAIL — `AttributeError: ... has no attribute 'apply_node_classification'`.
...
# backend/app/domain/ports.py — добавить в EstimateRepository (импортнуть ClassifiableNode)
    ...
    def apply_node_classification(
        self, node_id: int, *, excluded: bool, embedding_input: str
    ) -> None:
        """Пишет новую крошку; при excluded=True → status='excluded' (обратимо)."""
        ...
```
...
# импортировать ClassifiableNode из app.domain.entities в шапке.
    ...
    def apply_node_classification(
        ...
        values: dict = {"embedding_input": embedding_input}
        if excluded:
...
> **Note (реальный репозиторий):** `SqlAlchemyEstimateRepository` юнит-тестами не покрыт (нет БД в юнитах). Изменения (3b) зеркалят логику фейка; верификация — ручной прогон сметы (раздел «Финальная проверка»). Миграция НЕ нужна (`excluded` — новое значение существующей `VARCHAR`-колонки `status`).
...
git commit -m "$(printf 'feat(estimates): persistence excluded + чистка крошки + исключение из выборок\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>')"
```
...
**Interfaces:**
- Consumes: `WorkTypeClassifier`, `classify_lexical`, `build_embedding_input`, `fetch_all_nodes`, `apply_node_classification`, `NodeToClassify`, `WorkClass`.
...
# (4) новый метод:
    def _classify_nodes(self, estimate_id: int) -> None:
        ...
        # Проход 2: persist (excluded для ORG) + пересборка крошки (ORG-предки выброшены).
        for n in nodes:
            ancestors = [
...
**Type consistency:** `WorkClass`/`NodeToClassify`/`ClassifiableNode` определены в Tasks 3/5/7 до использования в Task 8; `classify(items)->list[WorkClass]` единообразно; `apply_node_classification(node_id, *, excluded, embedding_input)` совпадает в порту, репозитории и фейке; `fetch_matchable_nodes`/`count_*` не трогаются — `excluded` вне их наборов статусов.
