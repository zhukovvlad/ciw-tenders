...
6. Открыть смету, созданную ДО миграции 0009 (обе сметы текущей dev-БД ей
   удовлетворяют — `structure_anomalies` там `NULL` после `ALTER TABLE`) —
   страница не падает, StructureNotice отсутствует/пуст. Закрывает NULL-ветку
   `_to_entity`, оставленную непокрытой юнит-тестом в Task 1
   (`test_get_estimate_defaults_anomalies_to_empty_without_upload`).
