...
- **контраст на sunken-заливке** `#110D1C`: `font-medium` открывашки и
  `text-muted-foreground` (section_code слева / статусы справа) различимы —
  при провале поднять тон (спека §6, риск контраста);
...
- разделитель раздела и стилизация открывашки различимы на утопленном фоне
  (семантика границы из 3.5 не изменилась — проверка визуальная).
...
- **§4 тесты:** подпись рендерится (Task 1); строки без словаря кандидата
  ...
  тинт + маркер, неактивная — резерв прозрачным тоном (Task 3). Существующие
...
- **§4 живой гейт → Task 4** (смета 16 + однородный раздел С5_MIND: справка vs
  действие, правый столбец, контраст открывашки на sunken, воздух, кромка без
  «дёрга»).
