...
**Почему НЕ shadcn `Breadcrumb` (осознанное отступление от shadcn-first):** примитив существует (`BreadcrumbList`/`BreadcrumbEllipsis`), но он навигационный — рендерит `<nav aria-label="breadcrumb">` со списком ссылок. Наша крошка — неинтерактивная подпись, повторяющаяся десятками на одном экране (строка + рекомендация + каждый кандидат + каждый хит поиска): множить nav-ландмарки на декоративном тексте — a11y-антипаттерн (шум ландмарков для скринридера), а ссылок в крошке нет и не будет. Инлайновый `<span>` с DS-токенами — честнее. НЕ вендорить `breadcrumb.tsx`.
...
| shadcn-first: Card/Command/Badge/Button/Alert (5), Tabs/AlertDialog (9), Button (8); div-грид в языке Table — требование спеки §3c (виртуализация); отказ от `Breadcrumb` обоснован (4); `ui/` руками не править, вендоринг только CLI | 4, 5, 7, 8, 9 + Global Constraints |
