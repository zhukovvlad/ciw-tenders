...
- **`POST /api/estimates/{id}/match`** — ре-триггер матчинга. Ставит `match_estimate(id)`,
  **разрешён из ЛЮБОГО статуса своей сметы** (в т.ч. `running` — закрывает «воркер умер посреди
  матча» без реапера; в т.ч. `ready` — доматчивает `no_match`-строки после расширения справочника,
  при этом matchable-фильтр держит `confident/needs_review` иммутабельными). Без гейта по статусу:
  идемпотентность + matchable-фильтр делают повтор безопасным из любого состояния. Владение
