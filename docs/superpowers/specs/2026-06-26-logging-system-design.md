...
- **`root.setLevel(logging.DEBUG)` — ОБЯЗАТЕЛЬНО (иначе тихая дыра).** Root гейтит запись
  **первым** (`Logger.isEnabledFor` отсекает до `callHandlers`); дефолт root — WARNING. Без
  явного пола `app.log` (DEBUG-хендлер) не получит ничего ниже WARNING, консоль при
  `LOG_LEVEL=INFO` будет пустой, а per-estimate / AI-INFO из Секции 3 не запишутся вообще.
  Пол root = минимум по хендлерам (DEBUG); per-sink фильтрацию задают уровни хендлеров — на
  этом паттерне дизайн хендлеров и работает. Усмирение шумных либ не страдает: они гейтят
  записи у себя до пропагации в root.
- **Формат:**
...
- **`enqueue_articles_embed` — намеренно ВНЕ request-корреляции.** `embed_articles_task` —
  fan-in drain (тянет все pending-статьи из запросов A+B+C разом), один `request_id` на нём
  семантически размыт. Коррелируется только по `task_id`. Асимметрия «один enqueue с
  заголовком, другой без» — осознанная, не баг.
...
- **Логирование на ВСЕХ путях** (`try/finally`, не только после успешного `return`):
  ...
  ровно когда нужнее. **Ре-рейз обязателен (load-bearing):** классификатор полагается на свой
  внешний `except Exception → UNSURE`-фолбэк; swallow в `instrumented_call` тихо сломает его.
  Следствие — на сбое батча классификатора две строки (summary вызова + доменное «→ UNSURE»):
  это норма, не дубль.
