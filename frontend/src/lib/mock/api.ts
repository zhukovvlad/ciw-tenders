...
export async function matchEstimate(
  ...
  for (let i = 1; i <= total; i++) {
    ...
    const remainingReview = Math.max(
      0,
      review - Math.round((i / total) * review)
    )
    onProgress({
      phase: "matching",
      done: i,
      total,
      etaSeconds: remainingReview * 0.4,
    })
    await delay(20)
  }
...
export function exportEstimateCsv(state: ReviewState): string {
  ...
  for (const row of state.rows) {
    ...
    const score = manual || d.kind === "no_match" ? "" : row.score === null ? "" : row.score.toFixed(2)
    const alt2 = row.candidates[1]
      ? `${row.candidates[1].article_code} ${row.candidates[1].name}`
      : ""
    const alt3 = row.candidates[2]
      ? `${row.candidates[2].article_code} ${row.candidates[2].name}`
      : ""
    lines.push(
      [
